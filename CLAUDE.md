...
| Date | Changes |
| --- | --- |
| 2026-07-27 | Expanded the Custom button and popup-link styling controls in the Theme Editor. The `Style: Custom` panel's old "Button colors" group became four groups — Appearance, Borders (none/solid → thickness, opacity, color, corner radius), Typography (font, size, line height, letter spacing, case, text color) and Padding — applied to all five blocks that share `snippets/button-custom-styles.liquid` (`button`, `add-to-cart`, `contact-form-submit-button`, `email-signup`, `_product-list-button`). New ids are prefixed `custom_button_*` because `email-signup` already uses `border_*`/`padding-*` for its own container; the three pre-existing color ids were kept so no merchant data was lost. `button-custom-styles.liquid` now emits those styles in a doubled-class rule (specificity 0,2,0) placed before the `:hover` rule, and feeds padding through the `--button-padding-*` variables rather than physical properties so host blocks that deliberately zero button padding (email signup's arrow and integrated variants) keep working. Extracted the fluid `clamp()` font-size math out of `snippets/typography-style.liquid` into a new `snippets/fluid-font-size.liquid` shared by both. Rebuilt `blocks/popup-link.liquid`'s schema, which the 2026-07-28 Shopify sync (`326c730`) had deleted wholesale, and — for the first time — actually wired it up: the trigger previously rendered only `spacing-style`, so its typography/appearance settings had never done anything. Defaults throughout are visually neutral (padding 16/24 matches `--button-padding-*`, corner radius 100 matches `button_border_radius_primary`, borders default to none matching `secondary_button_border_width: 0`). `shopify theme check` passes clean. Not yet verified in a browser — see the verification steps in the session plan. |
