...
| Date | Changes |
| --- | --- |
| 2026-09-23 | Squash-merged `dev` into `main` (resolved an add/add conflict on `templates/page.contact.json` by keeping `dev`'s built-out contact form over `main`'s unedited placeholder), then reset `dev` to `main` and force-pushed to collapse `dev`'s 528 accumulated Shopify-editor autosave commits down to `main`'s clean 18. Added SEO changes on `dev`: `templates/robots.txt.liquid` (passthrough of `robots.default_groups`, crawling stays open) plus a sitewide `noindex, follow` meta tag in `snippets/meta-tags.liquid` — keeps the store out of search engine indexes without blocking crawling or breaking Open Graph/Twitter link previews, since robots.txt alone can't prevent indexing (confirmed via Shopify's own docs). Verified both via `shopify theme check` (clean) and a live `shopify theme dev` preview against `atx-prod-test-01`. |
